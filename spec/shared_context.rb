...
class TelefonicaVCR
  ...
  def initialize(options)
    ...
    if use_recorded
      ...
      @os_auth_url = if [Fog::Identity::Telefonica::V3,
                         Fog::Volume::Telefonica,
                         Fog::Volume::Telefonica::V1,
                         Fog::Volume::Telefonica::V2,
                         Fog::Image::Telefonica,
                         Fog::Image::Telefonica::V1,
                         Fog::Network::Telefonica,
                         Fog::DNS::Telefonica,
                         Fog::SharedFileSystem::Telefonica,
                         Fog::Monitoring::Telefonica].include? @service_class
                       'http://devstack.telefonica.stack:5000/v3'
                     else
                       'http://devstack.telefonica.stack:5000/v2.0'
                     end
    else
    ...
    VCR.use_cassette('common_setup') do
      ...
      if @service_class == Fog::Identity::Telefonica::V3 || @os_auth_url.end_with?('/v3')
        ...
        connection_options = {
          :telefonica_auth_url  => "#{@os_auth_url}/tokens",
          :telefonica_region    => @region,
