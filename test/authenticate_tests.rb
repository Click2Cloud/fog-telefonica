...
describe "Telefonica authentication" do
  ...
  it "with v2" do
    ...
    assert(expected) do
      Fog::Telefonica.authenticate_v2(
        :telefonica_auth_uri     => URI('http://example/v2.0/tokens'),
  ...
  it "v2 missing storage service" do
    ...
    proc do
      Fog::Telefonica.authenticate_v2(
        :telefonica_auth_uri     => URI('http://example/v2.0/tokens'),
        :telefonica_tenant       => 'admin',
        :telefonica_service_type => %w[object-store]
      )
  ...
  it "v2 auth with two compute services" do
    ...
    assert("http://example2:8774/v2/#{@tenant_token}") do
      Fog::Telefonica.authenticate_v2(
        ...
        :telefonica_tenant       => 'admin',
        :telefonica_service_type => %w[compute],
        :telefonica_service_name => 'nova2')[:server_management_url]
    end
  end
  ...
  it "legacy v1 auth" do
    ...
    assert("https://swift.myhost.com/v1/AUTH_tenant") do
      Fog::Telefonica.authenticate_v1(
        :telefonica_auth_uri     => URI('https://swift.myhost.com/auth/v1.0'),
        :telefonica_username     => 'tenant:dev',
        :telefonica_api_key      => 'secret_key',
        :telefonica_service_type => %w[storage])[:server_management_url]
    end
  end
