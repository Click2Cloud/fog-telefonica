...
describe "Shindo.tests('Fog::Compute[:telefonica] | service requests" do
  before do
    @service_format = {
      "id"              => Integer,
      "binary"          => String,
    ...
    @services = Fog::Compute[:telefonica].list_services.body
  ...
  describe "success" do
    ...
    it "#disable_service" do
      Fog::Compute[:telefonica].disable_service(
        @service['host'], @service['binary']
      ).body["service"]["status"].must_equal "disabled"
    end

    it "#disable_service_log_reason" do
      disabled_service = Fog::Compute[:telefonica].disable_service_log_reason(
    ...
    it "#enable_service" do
      Fog::Compute[:telefonica].enable_service(
        @service['host'], @service['binary']
