...
describe "Fog::Compute[:telefonica] | limits requests" do
  before do
    @rate_limit_format = {
      'regex' => String,
      'uri'   => String,
  ...
  describe "success" do
    describe "#get_limits" do
      it "format" do
        Fog::Compute[:telefonica].get_limits.body['limits'].
          must_match_schema(@limits_format)
      end

      it "rate limit format" do
        Fog::Compute[:telefonica].get_limits.body['limits']['rate'].
      ...
      it "absolute limits format" do
        Fog::Compute[:telefonica].get_limits.body['limits']['absolute'].
