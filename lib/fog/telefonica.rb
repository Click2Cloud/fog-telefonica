...
module Fog
  ...
  module Telefonica
    ...
    def self.retrieve_tokens_v3(options, connection_options = {})
      ...
      response, expires = Fog::Telefonica.token_cache[{:body => request_body, :path => path}] if cache_ttl > 0
      ...
      unless response && expires > Time.now
        ...
        if cache_ttl > 0
          cache = Fog::Telefonica.token_cache
          cache[{:body => request_body, :path => path}] = response, Time.now + cache_ttl
          Fog::Telefonica.token_cache = cache
        end
      end
